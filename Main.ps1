...
$stateMap = @(
    ...
    # ── Network ──
    @{ Chk='ChkNetThrottle';  State={ Test-RegState 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile' 'NetworkThrottlingIndex' 0xFFFFFFFF } }
    @{ Chk='ChkLLMNR';        State={ Test-RegState 'HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient' 'EnableMulticast' 0 } }
    @{ Chk='ChkMDNS';         State={ Test-RegState 'HKLM:\SYSTEM\CurrentControlSet\Services\Dnscache\Parameters' 'EnableMDNS' 0 } }
    ...
    # ── Windows Tweaks ──
    @{ Chk='ChkAnimations';   State={ Test-RegState 'HKCU:\Control Panel\Desktop' 'UserPreferencesMask' $null } }  # Sadece MenuDelay kontrolu yeterli
    ...
    @{ Chk='ChkNumlock';      State={ Test-RegState 'HKCU:\Control Panel\Keyboard' 'InitialKeyboardIndicators' '2147483650' } }
    @{ Chk='ChkLongPaths';    State={ Test-RegState 'HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem' 'LongPathsEnabled' 1 } }
)

$appliedCount = 0
$notAppliedCount = 0
$greenBrush = New-Brush '#4CAF50'   # Yesil — zaten uygulanmis
$defaultBrush = New-Brush '#E0E0E0' # Varsayilan checkbox rengi
foreach ($entry in $stateMap) {
    ...
    if ($ctrl) {
        try {
            ...
            if ($isApplied) {
                # Zaten uygulanmis — checkbox isaretli + yesil renk + tooltip guncelle
                $ctrl.IsChecked  = $true
                ...
                # Uygulanmamis — varsayilan rengi koru
...
# TCP AutoTuning ozel kontrol — netsh komutuyla
try {
    ...
    if ($atCtrl) {
        if ($tcpGlobal -match 'Receive Window Auto-Tuning Level\s*:\s*normal') {
            $atCtrl.IsChecked  = $true
            $atCtrl.Foreground = $greenBrush
            $existingTip = if ($atCtrl.ToolTip) { "$($atCtrl.ToolTip)`n" } else { '' }
            $atCtrl.ToolTip = "${existingTip}✓ Bu ayar zaten uygulanmis."
        }
    }
} catch {}
