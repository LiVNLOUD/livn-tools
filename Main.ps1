...
# Herhangi bir path var mi (wildcard destekli)
function Test-AnyPath([string[]]$Paths) {
    foreach ($p in $Paths) {
        try { if (Test-Path $p -ErrorAction SilentlyContinue) { return $true } } catch {}
    }
    return $false
}
...
function Test-App {
    param([string[]]$RegNames, [string[]]$Paths = @(), [string[]]$Publishers = @())
    if ($RegNames  -and (Test-RegName $RegNames))          { return $true }
...
$global:AppInstalled = @{
    ...
    'Opera'      = Test-App `
        ...
        -Publishers @('Opera Software') `
        -Paths      @("$env:LOCALAPPDATA\Programs\Opera\opera.exe")

    'OperaGX'    = Test-App `
        -RegNames   @('Opera GX') `
        -Paths      @("$env:LOCALAPPDATA\Programs\Opera GX\opera.exe")
...
$HWTimer.Add_Tick({
    ...
    if ($global:_UsbTickCounter % 2 -eq 0) {
        try {
            ...
            # Degisiklik algilama
            $currentIds = ($usbDrives | ForEach-Object { $_.DeviceID }) -join ','
            if ($currentIds -ne $global:_UsbLastIds) {
                ...
                if ($usbCount -gt 0) {
                    ...
                    $usbDrives | ForEach-Object { Write-Log "USB Algilandi: $($_.Model) [$($_.DeviceID)]" 'WARN' }
                } else {
...
$BtnRunQuickClean.Add_Click({
    ...
    $procChecks = @(
        ...
        @{ Key='Opera';     Proc=@('opera','opera_gx_setup');  Name='Opera / Opera GX'   }
    ...
    foreach ($chk in $procChecks) {
        if ($opts[$chk.Key]) {
            $running = $chk.Proc | ForEach-Object { Get-Process -Name $_ -ErrorAction SilentlyContinue } | Where-Object { $_ }
            if ($running) {
...
$BtnApplyPerf.Add_Click({
    $opts = @{
        ...
        WUpdate      = (Get-CheckVal 'ChkWUpdate')
        ...
        CpuIsAMD     = $global:HW.CpuIsAMD
        CpuIsIntel   = $global:HW.CpuIsIntel
        GpuIsNV      = $global:HW.GpuIsNV
        GpuIsAMD     = $global:HW.GpuIsAMD
    }
    ...
    Invoke-Async -TaskName 'Performance' -ProgressKey 'Perf' -Vars @{
        ...
        if ($opts.EmptyRAM -or $opts.ModifiedRAM) {
            ...
            if (-not (Test-Path $islcPath)) {
                Write-Log "ISLC bulunamadi: $islcPath" 'WARN'
            } else {
                ...
                # ISLC, kendi config'inden ayarlari okur; biz TaskScheduler olusturmuyoruz
                # ISLC bunu gorup kendi Task Scheduler kaydini yapar
                ...
                # ISLC'yi minimized olarak baslat — config'i okuyacak, Task Scheduler'a ekleyecek
...
$BtnApplyNetwork.Add_Click({
    $opts = @{
        ...
        AdaptInter  = (Get-CheckVal 'ChkAdaptInter')
    ...
    Invoke-Async -TaskName 'Network Tweaks' -ProgressKey 'Net' -Vars @{ opts=$opts } -Block {
        ...
        if ($opts.AutoTuning) {
            ...
            Write-Log "TCP AutoTuning: disabled" 'OK'
        ...
        if ($opts.Congestion) {
            ...
            Write-Log "Congestion Provider: CTCP (Compound TCP) aktif" 'OK'
        }
        if ($opts.NetThrottle) {
            $mmPath = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile'
            Set-ItemProperty -Path $mmPath -Name 'NetworkThrottlingIndex' -Value 0xFFFFFFFF -ErrorAction SilentlyContinue
        ...
        if ($opts.FlowCtrl -or $opts.IntMod -or $opts.RSS -or $opts.GreenEth -or $opts.GigaLite -or $opts.AdaptInter) {
            ...
            foreach ($adapter in $adapters) {
                ...
                if ($opts.IntMod)    { Set-NicProp 'Interrupt Moderation' 'Disabled' }
                if ($opts.RSS) {
        ...
        if ($opts.DNSPrefetch) {
            ...
            Set-ItemProperty -Path $dnsCachePath -Name 'MaxCacheTtl'           -Value 86400 -Type DWord -EA SilentlyContinue
            ...
            Write-Log "DNS Cache optimizasyonu uygulandı (MaxTTL=86400, NegTTL=0)" 'OK'
        }
        if ($opts.MDNS) {
...
$BtnApplyKernel.Add_Click({
    $opts = @{
        ...
        PagingFiles = (Get-CheckVal 'ChkPagingFiles')
    }
    Invoke-Async -TaskName 'Kernel & Input' -ProgressKey 'Kern' -Vars @{ opts=$opts } -Block {
        ...
        if ($opts.SecLvlCache) {
            ...
            # SecondLevelDataCache = en yuksek seviye onbellek (L3 > L2 > fallback)
            $cacheKB = 512
            if ($cpu) {
                if ($cpu.L3CacheSize -gt 0)     { $cacheKB = [int]$cpu.L3CacheSize }
                elseif ($cpu.L2CacheSize -gt 0) { $cacheKB = [int]$cpu.L2CacheSize }
            }
...
$BtnApplyGPU.Add_Click({
    $opts = @{
        ...
        NvShaderCache = (Get-CheckVal 'ChkNvShaderCache')
        NvTexFilter   = (Get-CheckVal 'ChkNvTexFilter')
        NvFastSync   = (Get-CheckVal 'ChkNvFastSync')
        AMDAntiLag   = (Get-CheckVal 'ChkAMDAntiLag')
        AMDChill     = (Get-CheckVal 'ChkAMDChill')
        AMDPower     = (Get-CheckVal 'ChkAMDPower')
    }
...
$BtnApplyWinTweaks.Add_Click({
    $opts = @{
        ...
        Transparency = (Get-CheckVal 'ChkTransparency')
        CrashDumpFull = (Get-CheckVal 'ChkCrashDumpFull')
        EventLogSize  = (Get-CheckVal 'ChkEventLogSize')
    }
...
$PresetMinimal = @{
    ...
    ChkRawInput=$false; ChkLargePages=$false; ChkContMem=$false; ChkSecondLevelCache=$false; ChkPagingFiles=$false
    # GPU - safe
...
$PresetStandard = @{
    ...
    # Kernel
    ChkVBS=$false; ChkDMAProtect=$false; ChkSpectre=$false; ChkCFG=$false; ChkHVCI=$false
    ChkLargePages=$false; ChkContMem=$true; ChkSecondLevelCache=$true; ChkPagingFiles=$false
...
$PresetAggressive = @{
    ...
    # Kernel
    ChkVBS=$false; ChkDMAProtect=$false; ChkSpectre=$false; ChkCFG=$false; ChkHVCI=$false
    ChkLargePages=$false; ChkContMem=$true; ChkSecondLevelCache=$true; ChkPagingFiles=$true
    ChkMouseBuffer=$true; ChkKbBuffer=$true; ChkMouseSmooth=$true; ChkMouseAccel=$true; ChkRawInput=$true
    # GPU
...
$BtnApplyAll.Add_Click({
    ...
    # Her sayfanin opts hashtable'ini al
    $allPerfOpts = @{
        HPET=($Window.FindName('ChkHPET').IsChecked -eq $true); TimerRes=($Window.FindName('ChkTimerRes').IsChecked -eq $true)
        CpuPrio=($Window.FindName('ChkCpuPriority').IsChecked -eq $true); SysMain=($Window.FindName('ChkSysMain').IsChecked -eq $true)
        WSearch=($Window.FindName('ChkWSearch').IsChecked -eq $true); WUpdate=($Window.FindName('ChkWUpdate').IsChecked -eq $true)
        GameMode=($Window.FindName('ChkGameMode').IsChecked -eq $true); HwAccel=($Window.FindName('ChkHwAccel').IsChecked -eq $true)
        EmptyRAM=($Window.FindName('ChkEmptyRAM').IsChecked -eq $true); ModifiedRAM=($Window.FindName('ChkModifiedRAM').IsChecked -eq $true)
        CpuIsAMD=$global:HW.CpuIsAMD; CpuIsIntel=$global:HW.CpuIsIntel; GpuIsNV=$global:HW.GpuIsNV; GpuIsAMD=$global:HW.GpuIsAMD
        planAction=$null
    }
    ...
    $allNetOpts = @{
        AutoTuning=($Window.FindName('ChkAutoTuning').IsChecked -eq $true); ECN=($Window.FindName('ChkECN').IsChecked -eq $true)
        RSC=($Window.FindName('ChkRSC').IsChecked -eq $true); Congestion=($Window.FindName('ChkCongestion').IsChecked -eq $true)
        NetThrottle=($Window.FindName('ChkNetThrottle').IsChecked -eq $true); Nagle=($Window.FindName('ChkNagle').IsChecked -eq $true)
        TCPNoDelay=($Window.FindName('ChkTCPNoDelay').IsChecked -eq $true); TCPACKFreq=($Window.FindName('ChkTCPACKFreq').IsChecked -eq $true)
        RSS=($Window.FindName('ChkRSS').IsChecked -eq $true); FlowCtrl=($Window.FindName('ChkFlowCtrl').IsChecked -eq $true)
        IntMod=($Window.FindName('ChkIntMod').IsChecked -eq $true); GreenEth=($Window.FindName('ChkGreenEth').IsChecked -eq $true)
        GigaLite=($Window.FindName('ChkGigaLite').IsChecked -eq $true); AdaptInter=($Window.FindName('ChkAdaptInter').IsChecked -eq $true)
        DNSPrefetch=($Window.FindName('ChkDNSPrefetch').IsChecked -eq $true); MDNS=($Window.FindName('ChkMDNS').IsChecked -eq $true)
        LLMNR=($Window.FindName('ChkLLMNR').IsChecked -eq $true)
    }
    $allKernOpts = @{
        VBS=($Window.FindName('ChkVBS').IsChecked -eq $true); DMAProtect=($Window.FindName('ChkDMAProtect').IsChecked -eq $true)
        Spectre=($Window.FindName('ChkSpectre').IsChecked -eq $true); CFG=($Window.FindName('ChkCFG').IsChecked -eq $true)
        HVCI=($Window.FindName('ChkHVCI').IsChecked -eq $true); LargePages=($Window.FindName('ChkLargePages').IsChecked -eq $true)
        ContMem=($Window.FindName('ChkContMem').IsChecked -eq $true); SecLvlCache=($Window.FindName('ChkSecondLevelCache').IsChecked -eq $true)
        MouseBuf=($Window.FindName('ChkMouseBuffer').IsChecked -eq $true); KbBuf=($Window.FindName('ChkKbBuffer').IsChecked -eq $true)
        RawInput=($Window.FindName('ChkRawInput').IsChecked -eq $true); MouseSmooth=($Window.FindName('ChkMouseSmooth').IsChecked -eq $true)
        MouseAccel=($Window.FindName('ChkMouseAccel').IsChecked -eq $true); PagingFiles=($Window.FindName('ChkPagingFiles').IsChecked -eq $true)
    }
    $allGpuOpts = @{
        MSIGPU=($Window.FindName('ChkMSIGPU').IsChecked -eq $true); MSINVMe=($Window.FindName('ChkMSINVMe').IsChecked -eq $true)
        MSINIC=($Window.FindName('ChkMSINIC').IsChecked -eq $true); MSIPRIO=($Window.FindName('ChkMSIPrio').IsChecked -eq $true)
        NvPrerender=($Window.FindName('ChkNvPrerender').IsChecked -eq $true); NvPower=($Window.FindName('ChkNvPower').IsChecked -eq $true)
        NvSync=($Window.FindName('ChkNvSync').IsChecked -eq $true); NvShaderCache=($Window.FindName('ChkNvShaderCache').IsChecked -eq $true)
        NvTexFilter=($Window.FindName('ChkNvTexFilter').IsChecked -eq $true); NvFastSync=($Window.FindName('ChkNvFastSync').IsChecked -eq $true)
        AMDAntiLag=($Window.FindName('ChkAMDAntiLag').IsChecked -eq $true); AMDChill=($Window.FindName('ChkAMDChill').IsChecked -eq $true)
        AMDPower=($Window.FindName('ChkAMDPower').IsChecked -eq $true)
    }
    $allPrivOpts = @{
        DiagTrack=($Window.FindName('ChkDiagTrack').IsChecked -eq $true); DMWApp=($Window.FindName('ChkDMWAppSupport').IsChecked -eq $true)
        TelemetryReg=($Window.FindName('ChkTelemetryReg').IsChecked -eq $true); AppCompat=($Window.FindName('ChkAppCompat').IsChecked -eq $true)
        ErrReport=($Window.FindName('ChkErrorReport').IsChecked -eq $true); ActHist=($Window.FindName('ChkActivityHist').IsChecked -eq $true)
        Cortana=($Window.FindName('ChkCortana').IsChecked -eq $true); AdID=($Window.FindName('ChkAdID').IsChecked -eq $true)
        Tailored=($Window.FindName('ChkTailored').IsChecked -eq $true); Typing=($Window.FindName('ChkTyping').IsChecked -eq $true)
        CEIP=($Window.FindName('ChkCEIP').IsChecked -eq $true); OneDrive=($Window.FindName('ChkOneDrive').IsChecked -eq $true)
        XboxSvc=($Window.FindName('ChkXboxServices').IsChecked -eq $true); BingSearch=($Window.FindName('ChkBingSearch').IsChecked -eq $true)
        SuggestApps=($Window.FindName('ChkSuggestApps').IsChecked -eq $true); ConsumerExp=($Window.FindName('ChkConsumerExp').IsChecked -eq $true)
    }
    $allWinOpts = @{
        Animations=($Window.FindName('ChkAnimations').IsChecked -eq $true); JPEGQuality=($Window.FindName('ChkJPEGQuality').IsChecked -eq $true)
        MenuDelay=($Window.FindName('ChkMenuDelay').IsChecked -eq $true); TaskbarAnims=($Window.FindName('ChkTaskbarAnims').IsChecked -eq $true)
        BSODDetail=($Window.FindName('ChkBSODDetail').IsChecked -eq $true); LaunchTo=($Window.FindName('ChkLaunchTo').IsChecked -eq $true)
        NumLock=($Window.FindName('ChkNumlock').IsChecked -eq $true); HideExt=($Window.FindName('ChkHideExt').IsChecked -eq $true)
        LongPaths=($Window.FindName('ChkLongPaths').IsChecked -eq $true); ContextMenu=($Window.FindName('ChkContextMenu').IsChecked -eq $true)
        DarkMode=($Window.FindName('ChkDarkMode').IsChecked -eq $true); Transparency=($Window.FindName('ChkTransparency').IsChecked -eq $true)
        CrashDumpFull=($Window.FindName('ChkCrashDumpFull').IsChecked -eq $true); EventLogSize=($Window.FindName('ChkEventLogSize').IsChecked -eq $true)
    }
    ...
    Invoke-Async -TaskName 'Tum Optimizasyonlar' -Vars @{
        ...
        # ISLC — EmptyRAM/ModifiedRAM seciliyse
        if ($opts.EmptyRAM -or $opts.ModifiedRAM) {
            $islcExeAA  = 'Intelligent standby list cleaner ISLC.exe'
            $islcPathAA = Join-Path $RootPath "_Files\ISLC\$islcExeAA"
            if (-not (Test-Path $islcPathAA)) {
                Write-Log "ISLC bulunamadi: $islcPathAA" 'WARN'
            } else {
        ...
        $opts = $allNetOpts
        if ($opts.AutoTuning) { netsh int tcp set global autotuninglevel=disabled 2>&1|Out-Null; Write-Log "AutoTuning disabled" 'OK' }
        ...
        if ($opts.Congestion) { netsh int tcp set supplemental template=internet congestionprovider=ctcp 2>&1|Out-Null; netsh int tcp set supplemental template=internetcustom congestionprovider=ctcp 2>&1|Out-Null; Write-Log "CTCP aktif" 'OK' }
        ...
        if ($opts.FlowCtrl -or $opts.IntMod -or $opts.RSS -or $opts.GreenEth -or $opts.GigaLite -or $opts.AdaptInter) {
            ...
            foreach ($nic in $nicAdapters) {
                ...
                if ($opts.IntMod)    { Set-NicP 'Interrupt Moderation' 'Disabled' }
        ...
        if ($opts.DNSPrefetch) {
            ...
            Set-ItemProperty $dc 'MaxCacheTtl' 86400 -Type DWord -EA SilentlyContinue
            ...
            Write-Log "DNS Cache optimizasyonu (MaxTTL=86400)" 'OK'
        }
        ...
        if ($opts.SecLvlCache) {
            $cpu2=Get-CimInstance Win32_Processor -EA SilentlyContinue | Select-Object -First 1
            $ck=512; if($cpu2){if($cpu2.L3CacheSize -gt 0){$ck=[int]$cpu2.L3CacheSize} elseif($cpu2.L2CacheSize -gt 0){$ck=[int]$cpu2.L2CacheSize}}
            Set-ItemProperty 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management' 'SecondLevelDataCache' $ck -EA SilentlyContinue
            Write-Log "SecondLevelDataCache=${ck}KB" 'OK'
        }
...
Write-Log "  CPU : $($global:HW.CpuName) ($($global:HW.CpuCores)C/$($global:HW.CpuThreads)T)" 'INFO'
...
# USB ilk tarama
try {
    $initUsb = @(Get-CimInstance Win32_DiskDrive -ErrorAction SilentlyContinue | Where-Object { $_.InterfaceType -eq 'USB' })
    $global:_UsbLastIds = ($initUsb | ForEach-Object { $_.DeviceID }) -join ','
    if ($initUsb.Count -gt 0) {
        ...
        $initUsb | ForEach-Object { Write-Log "  - $($_.Model)" 'WARN' }
    } else {
        $TxtUsbCount.Text       = 'Yok'
        $TxtUsbCount.Foreground = [System.Windows.Media.Brushes]::Gray
    }
...
if ($detectedApps) { Write-Log "  Bulunan: $detectedApps" 'INFO' }
$notFound = ($appCheckMap | Where-Object { -not $global:AppInstalled[$_.Key] } | ForEach-Object { $_.Key }) -join ', '
if ($notFound) { Write-Log "  Bulunamadi (gri): $notFound" 'INFO' }
