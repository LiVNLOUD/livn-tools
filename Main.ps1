...
$global:AppInstalled = @{
    ...
    'Chrome'     = Test-App `
        ...
        -Paths      @("$env:LOCALAPPDATA\Google\Chrome\User Data",
                      "$env:ProgramFiles\Google\Chrome\Application\chrome.exe",
                      "${env:ProgramFiles(x86)}\Google\Chrome\Application\chrome.exe")

    'Brave'      = Test-App `
        -RegNames   @('Brave') `
        -Paths      @("$env:LOCALAPPDATA\BraveSoftware\Brave-Browser\User Data",
                      "$env:ProgramFiles\BraveSoftware\Brave-Browser\Application\brave.exe")

    'Firefox'    = Test-App `
        -RegNames   @('Mozilla Firefox','Firefox') `
        -Paths      @("$env:APPDATA\Mozilla\Firefox\Profiles",
                      "$env:ProgramFiles\Mozilla Firefox\firefox.exe",
                      "${env:ProgramFiles(x86)}\Mozilla Firefox\firefox.exe")

    'Edge'       = Test-App `
        -RegNames   @('Microsoft Edge') `
        -Paths      @("$env:LOCALAPPDATA\Microsoft\Edge\User Data\Default",
                      "$env:ProgramFiles\Microsoft\Edge\Application\msedge.exe")

    'Opera'      = Test-App `
        -RegNames   @('Opera Stable','Opera Browser','Opera') `
        -Publishers @('Opera Software') `
        -Paths      @("$env:APPDATA\Opera Software\Opera Stable",
                      "$env:LOCALAPPDATA\Programs\Opera\opera.exe")

    'OperaGX'    = Test-App `
        -RegNames   @('Opera GX') `
        -Paths      @("$env:APPDATA\Opera Software\Opera GX Stable",
                      "$env:LOCALAPPDATA\Programs\Opera GX\opera.exe")

    'Vivaldi'    = Test-App `
        -RegNames   @('Vivaldi') `
        -Paths      @("$env:LOCALAPPDATA\Vivaldi\User Data",
                      "$env:LOCALAPPDATA\Programs\Vivaldi\Application\vivaldi.exe")

    'Tor'        = Test-App `
        -RegNames   @('Tor Browser') `
        -Paths      @("$env:LOCALAPPDATA\Tor Browser\Browser\TorBrowser",
                      "$env:DESKTOP\Tor Browser\Browser\firefox.exe")

    # ── MESAJLASMA / ILETISIM ────────────────────────────────────────────────
    'Discord'    = Test-App `
        -RegNames   @('Discord') `
        -Paths      @("$env:LOCALAPPDATA\Discord",
                      "$env:APPDATA\discord")

    'Telegram'   = Test-App `
        -RegNames   @('Telegram','Telegram Desktop') `
        -Paths      @("$env:APPDATA\Telegram Desktop",
                      "$env:LOCALAPPDATA\Telegram Desktop")

    'WhatsApp'   = Test-App `
        -RegNames   @('WhatsApp') `
        -Paths      @("$env:LOCALAPPDATA\WhatsApp",
                      "$env:LOCALAPPDATA\Packages\5319275A.WhatsAppDesktop_cv1g1gvanyjgm")
    ...
    'EpicGames'  = Test-App `
        ...
        -Paths      @("$env:LOCALAPPDATA\EpicGamesLauncher\Saved",
    ...
    'UbisoftConnect' = Test-App `
        ...
        -Publishers @('Ubisoft') `
        -Paths      @("$env:LOCALAPPDATA\Ubisoft Game Launcher",
    ...
    'EADesktop'  = Test-App `
        ...
        -Paths      @("$env:PROGRAMDATA\Electronic Arts\EA Desktop",
                      "$env:LOCALAPPDATA\Electronic Arts\EA Desktop",
                      "$env:ProgramFiles\Electronic Arts\EA Desktop\EA Desktop.exe",
    ...
    'Xbox'       = Test-App `
        ...
        -Paths      @("$env:LOCALAPPDATA\Packages\Microsoft.GamingApp_8wekyb3d8bbwe",
    ...
    'Rockstar'   = Test-App `
        ...
        -Paths      @("$env:LOCALAPPDATA\Rockstar Games\Launcher",
                      "$env:ProgramFiles\Rockstar Games\Launcher\Launcher.exe",
                      "${env:ProgramFiles(x86)}\Rockstar Games\Launcher")

    'Minecraft'  = Test-App `
        -RegNames   @('Minecraft Launcher','Minecraft') `
        -Paths      @("$env:APPDATA\.minecraft",
                      "$env:LOCALAPPDATA\Packages\Microsoft.4297127D64EC6_8wekyb3d8bbwe")

    'Riot'       = Test-App `
        -RegNames   @('Riot Client','Riot Games','Riot Vanguard','League of Legends','VALORANT') `
        -Publishers @('Riot Games') `
        -Paths      @("$env:LOCALAPPDATA\Riot Games",
...
function Confirm-AppClosed([string]$processName, [string]$appDisplayName) {
    ...
    if ($proc) {
        $result = [System.Windows.MessageBox]::Show(
            "$appDisplayName su anda acik!`n`nTemizlik icin uygulamay kapatilmasi onerilir.`nYine de devam etmek istiyor musunuz?",
    ...
    if ($global:_UsbTickCounter % 2 -eq 0) {
        try {
            ...
            if ($currentIds -ne $global:_UsbLastIds) {
                ...
                if ($usbCount -gt 0) {
                    $label = if ($usbCount -eq 1) { "$usbCount Cihaz Bagli" } else { "$usbCount Cihaz Bagli" }
...
$BtnRunAdvClean.Add_Click({
    ...
    Invoke-Async -TaskName 'Advanced Clean' -Vars @{
        ...
        if ($opts.SuperFetch) {
            Advance-Step 'SysMain kapatiliyor...'
    ...
    Invoke-Async -TaskName 'Performance' -ProgressKey 'Perf' -Vars @{
        ...
        if ($opts.EmptyRAM -or $opts.ModifiedRAM) {
            ...
            if (-not (Test-Path $islcPath)) {
                ...
                # Monitoring — CLI parametreli, -Wait YOK
...
# ─── HELPER: HYBRED PLAN IMPORT (FIX 6) ──────────────────────────────────────
function Invoke-HybredPlan {
    param([string]$PowFileName)
    $planDirs = @(
        (Join-Path $global:RootPath '_Files\HybredPowerPlans'),
        (Join-Path $global:RootPath '_Files')
    )
    $planPath = $null
    foreach ($d in $planDirs) {
        $candidate = Join-Path $d $PowFileName
        if (Test-Path $candidate) { $planPath = $candidate; break }
    }
    if (-not $planPath) {
        Write-Log "Hybred plan dosyasi bulunamadi: $PowFileName" 'WARN'
        Write-Log "Aranan konumlar: $($planDirs -join ', ')" 'INFO'
        return
    }
    Write-Log "Hybred plan yukleniyor: $PowFileName" 'RUN'
    ...
    if ($guidMatch.Success) {
        ...
        Write-Log "Hybred Power Plan aktif edildi: $guid" 'OK'
        Write-Log "Dosya: $PowFileName" 'INFO'
    } else {
        Write-Log "Hybred plan GUID alinamadi. Import ciktisi: $($importOut -join ' ')" 'WARN'
    }
}
...
$BtnPlanHybred.Add_Click({
    ...
    Invoke-Async -TaskName 'Hybred HighPerf Plan' -Block {
        Invoke-HybredPlan 'HybredLowLatencyHighPerf.pow'
    }
})

$BtnPlanHybred2.Add_Click({
    Set-ActiveButton -ActiveBtn $BtnPlanHybred2 -Group $global:_PlanBtnGroup
    Invoke-Async -TaskName 'Hybred Balanced Plan' -Block {
        Invoke-HybredPlan 'HybredLowLatencyBalanced.pow'
    }
})
...
$BtnPlanBitsum.Add_Click({
    ...
    Invoke-Async -TaskName 'Guc Plani - Bitsum Highest' -Block {
        ...
        if (Test-Path $planPath) {
            ...
            $existing = powercfg /list 2>&1 | Where-Object { $_ -match 'e9a42b02-d5df-448d-aa00-03f14749eb61' }
            if ($existing) {
                ...
                powercfg /duplicatescheme e9a42b02-d5df-448d-aa00-03f14749eb61 2>&1 | Out-Null
                $dup = powercfg /list 2>&1 | Select-String '([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})' |
                       Where-Object { $_ -match 'e9a42b02' } | Select-Object -Last 1
                if ($dup) {
                    $g2 = ([regex]'([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})').Match($dup).Value
                    if ($g2) { powercfg /setactive $g2 2>&1 | Out-Null }
                }
...
$BtnApplyNetwork.Add_Click({
    ...
    Invoke-Async -TaskName 'Network Tweaks' -ProgressKey 'Net' -Vars @{ opts=$opts } -Block {
        ...
        if ($opts.FlowCtrl -or $opts.IntMod -or $opts.RSS -or $opts.GreenEth -or $opts.GigaLite) {
        ...
        if ($opts.DNSPrefetch) {
            # DNS Önbellek (Client) kapat — DNS tabanlı izlemeyi azalt
            ...
            Set-ItemProperty -Path $dnsCachePath -Name 'EnableDnsCache' -Value 0 -Type DWord -EA SilentlyContinue
            Write-Log "DNS Cache (Prefetch): disabled" 'OK'
        }
        if ($opts.MDNS) {
...
$BtnApplyGPU.Add_Click({
    ...
    Invoke-Async -TaskName 'GPU & MSI' -ProgressKey 'GPU' -Vars @{ opts=$opts; HW=$global:HW } -Block {
        $msiCount = 0
        if ($opts.MSIGPU -or $opts.MSINVMe) {
            ...
            Get-ChildItem -Path $pciPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                ...
                if (($opts.MSIGPU -and $isGPU) -or ($opts.MSINVMe -and $isNVMe)) {
                    if (-not (Test-Path $intPath)) { try { New-Item -Path $intPath -Force -EA SilentlyContinue | Out-Null } catch {} }
                    Set-ItemProperty -Path $intPath -Name 'MSISupported' -Value 1 -Type DWord -EA SilentlyContinue
                    if ($opts.MSIPRIO) {
                        $affinityPath = "$($_.PSPath)\Device Parameters\Interrupt Management\Affinity Policy"
                        if (-not (Test-Path $affinityPath)) { try { New-Item -Path $affinityPath -Force -EA SilentlyContinue | Out-Null } catch {} }
                        Set-ItemProperty -Path $affinityPath -Name 'DevicePriority' -Value 3 -Type DWord -EA SilentlyContinue
                    }
                    $script:msiCount++
                }
            }
            if ($msiCount -gt 0) {
                Write-Log "MSI Mode: $msiCount aygit icin etkinlestirildi (GPU/NVMe). Yeniden baslatma gerekli." 'OK'
                if ($opts.MSIPRIO) { Write-Log "  MSI IRQ Priority = High (DevicePriority=3)" 'OK' }
            } else {
                Write-Log "MSI Mode: Uygun PCI aygiti bulunamadi." 'WARN'
            }
        ...
        $nvSelected = $opts.NvPrerender -or $opts.NvPower -or $opts.NvSync -or $opts.NvFastSync
        if ($nvSelected) {
            if (Test-Path $nvPath) {
                $nvFound = $false
                Get-ChildItem $nvPath | Where-Object { $_.PSChildName -match '^\d{4}$' } | ForEach-Object {
                    ...
                    if ($driverDesc -match 'NVIDIA') {
                        ...
                        if ($opts.NvSync)      { $applied += 'Sync=off(registry)' }
        ...
        $amdSelected = $opts.AMDAntiLag -or $opts.AMDChill
        if ($amdSelected) {
...
$BtnBackupGPU.Add_Click({
    ...
    Write-Log "GPU & MSI yedekleri alindi: _Files\Backups klasoru" 'OK'
})
...
$BtnApplyAll.Add_Click({
    Invoke-Async -TaskName 'Apply All' -Block {
        ...
        Write-Log "Yedek almak icin once ilgili sayfadaki 'Backup' butonuna tiklayin." 'INFO'
...
Write-Log "─────────────────────────────────────────" 'INFO'
Write-Log "UYGULAMA TESPITI ($($global:_RegCache.Count) registry kaydi tarandy):" 'INFO'
